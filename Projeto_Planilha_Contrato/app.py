...
for linha in pagina_fornecedores.iter_rows(min_row=2,values_only=True):
   nome_empresa, endereço, cidade, estado, cep, telefone, email, setor = linha #unpacking
   print(nome_empresa)
   ...
   texto_contrato = f"""
   Este contrato de prestação de serviços é feito entre {nome_empresa}, com endereço em {endereço}, 
   ...
   CONTRATANTE: Layfon Prestações de Serviço SA
   E-mail: layfonserviços@gmail.com

   Salvador, {datetime.now().strftime('%d/%m/%Y')}
   """

   arquivo_word.add_paragraph(texto_contrato)

   arquivo_word.save(f'./contratos/contrato_{nome_empresa}.docx')
# - Salvar o arquivo word em uma pasta específica (Pasta Contratos)
